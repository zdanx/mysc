...
-- Loading UI Setup (Script tertahan selama 2.5 detik)
local function PlayLoadingAnimation(IconId, Duration)
	local CoreGui = game:GetService("CoreGui")
	local TweenService = game:GetService("TweenService")
	local HttpService = game:GetService("HttpService")
	
	-- Mendukung eksekutor dengan gethui() atau fallback ke CoreGui
	local TargetGui = (typeof(gethui) == "function" and gethui()) or CoreGui

	local LoadingGui = Instance.new("ScreenGui")
	LoadingGui.Name = HttpService:GenerateGUID(false)
	...
	Loader.Name = HttpService:GenerateGUID(false)
	...
	Icon.ZIndex = 100
	Icon.Image = IconId or "rbxassetid://75680508301013"
	Icon.ImageTransparency = 1

	local Vignette = Instance.new("ImageLabel")
	Vignette.Name = HttpService:GenerateGUID(false)
	Vignette.Parent = Loader
	Vignette.BackgroundTransparency = 1
	Vignette.BorderSizePixel = 0
	Vignette.Size = UDim2.new(1, 0, 1, 0)
	Vignette.Image = "rbxassetid://18720640102"
	Vignette.ImageColor3 = Color3.fromRGB(5, 69, 223)
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5, 0.5)
	Vignette.Position = UDim2.fromScale(0.5, 0.5)

	TweenService:Create(Loader, TweenInfo.new(0.55, Enum.EasingStyle.Quint), {BackgroundTransparency = 0.5}):Play()

	local Event = Instance.new('BindableEvent')

	task.delay(0.5, function()
		TweenService:Create(Icon, TweenInfo.new(0.75, Enum.EasingStyle.Quint), {ImageTransparency = 0.01, Size = UDim2.new(0, 360, 0, 360)}):Play()

		task.delay(0.25, function()
			TweenService:Create(Vignette, TweenInfo.new(5), {ImageTransparency = 0.2}):Play()

			task.wait(Duration or 2.5)

			TweenService:Create(Vignette, TweenInfo.new(3, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {Size = UDim2.new(2, 0, 2, 0)}):Play()
			TweenService:Create(Icon, TweenInfo.new(0.75, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {ImageTransparency = 1}):Play()
			TweenService:Create(Loader, TweenInfo.new(1.5, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {BackgroundTransparency = 1}):Play()

			task.delay(0.1, function()
				TweenService:Create(Vignette, TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {ImageTransparency = 1}):Play()
	...
	-- Yield (tahan script) hingga animasi selesai
	return Event.Event:Wait()
end

-- Panggil fungsi untuk menahan proses eksekusi
PlayLoadingAnimation("rbxassetid://75680508301013", 2.5)
...
task.spawn(function()
    ...
    while task.wait(0.5) do
        -- Melakukan iterasi untuk mencari frame Discord di dalam UI eksekutor
        for _, gui in ipairs(coreGui:GetChildren()) do
            local homeNode = gui:FindFirstChild("Home", true)
            
            if homeNode and homeNode:FindFirstChild("Interactions") then
                local discordFrame = homeNode.Interactions:FindFirstChild("Discord")
                
                if discordFrame then
                    -- Memaksa posisi dan ukuran sesuai nilai dari Dex Explorer
                    discordFrame.Position = UDim2.new(0, 447, 0, 129)
                    discordFrame.Size = UDim2.new(0, 287, 0, 90)
                    
                    -- Menghentikan loop setelah modifikasi berhasil
                    return
                end
            end
        end
...
task.spawn(function()
    ...
    -- Asumsi resolusi dasar layar kamu saat melihat/mengambil nilai dari Dex Explorer
    -- (1920x1080 adalah resolusi standar PC pada umumnya)
    local BASE_WIDTH = 1920
    local BASE_HEIGHT = 1080

    -- Fungsi untuk mengubah nilai piksel (Offset) menjadi ukuran responsif
    local function getResponsiveUDim2(offsetX, offsetY)
        local viewport = camera.ViewportSize
        local scaleX = viewport.X / BASE_WIDTH
        local scaleY = viewport.Y / BASE_HEIGHT
        
        -- Mengambil skala terkecil agar bentuk UI tetap proporsional (tidak gepeng)
        local scaleFactor = math.min(scaleX, scaleY)
        
        -- Nilai akhir dikalikan dengan scaleFactor perangkat yang sedang dipakai
        return UDim2.new(0, offsetX * scaleFactor, 0, offsetY * scaleFactor)
    end

    while task.wait(0.5) do
        -- Melakukan iterasi untuk mencari UI di dalam CoreGui
        for _, gui in ipairs(coreGui:GetChildren()) do
            local homeNode = gui:FindFirstChild("Home", true)
            
            if homeNode and homeNode:FindFirstChild("Interactions") then
                local interactions = homeNode.Interactions
                
                -- Mencari semua frame utama
                local discordFrame = interactions:FindFirstChild("Discord")
                local clientFrame = interactions:FindFirstChild("Client")
                local friendsFrame = interactions:FindFirstChild("Friends")
                local serverFrame = interactions:FindFirstChild("Server")
                local userFrame = interactions:FindFirstChild("User")
                
                -- Mengecek apakah semua frame sudah dimuat
                if discordFrame and clientFrame and friendsFrame and serverFrame and userFrame then
                    
                    -- Menggunakan fungsi getResponsiveUDim2(NilaiX, NilaiY) untuk Posisi & Ukuran
                    
                    -- 1. Memodifikasi frame Discord
                    discordFrame.Position = getResponsiveUDim2(448, 129)
                    discordFrame.Size = getResponsiveUDim2(287, 90)
                    local discordSubtitle = discordFrame:FindFirstChild("Subtitle")
                    if discordSubtitle then
                        discordSubtitle.Text = "Ketuk untuk bergabung dengan server Discord kami demi mendapatkan pembaruan dan berita."
                        discordSubtitle.TextScaled = true -- Menyesuaikan ukuran teks otomatis
                    end
                    
                    -- 2. Memodifikasi frame Client (Xeno)
                    clientFrame.Position = getResponsiveUDim2(511, 450)
                    clientFrame.Size = getResponsiveUDim2(328, 90)
                    local clientSubtitle = clientFrame:FindFirstChild("Subtitle")
                    if clientSubtitle then
                        clientSubtitle.Text = "Anda menjalankan klien yang didukung Sirius."
                        clientSubtitle.TextScaled = true
                    end
                    
                    -- 3. Memodifikasi frame Friends
                    friendsFrame.Position = getResponsiveUDim2(511, 228)
                    friendsFrame.Size = getResponsiveUDim2(371, 214)
                    local friendsSubtitle = friendsFrame:FindFirstChild("Subtitle")
                    if friendsSubtitle then
                        friendsSubtitle.Text = "Cari tahu apa yang sedang dilakukan teman-teman Anda saat ini."
                        friendsSubtitle.TextScaled = true
                    end
                    
                    -- 4. Memodifikasi frame Server
                    serverFrame.Position = getResponsiveUDim2(82, 228)
                    serverFrame.Size = getResponsiveUDim2(420, 283)
                    local serverSubtitle = serverFrame:FindFirstChild("Subtitle")
                    if serverSubtitle then
                        serverSubtitle.Text = "Informasi tentang sesi yang sedang Anda ikuti saat ini."
                        serverSubtitle.TextScaled = true
                    end
                    
                    -- 5. Memodifikasi frame User
                    userFrame.Position = getResponsiveUDim2(82, 129)
                    userFrame.Size = getResponsiveUDim2(356, 90)
                    
                    -- Menghentikan loop setelah semua modifikasi berhasil
                    return
                end
            end
        end
...
local function UpdateHome()
	...
	-- Home Title
	homeContainer.Title.Text = "Yaww welcome " .. localPlayer.DisplayName
